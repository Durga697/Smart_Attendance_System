...
# Data directory for persistent storage (e.g. Render Persistent Disk mount point /var/data)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.environ.get('DATA_DIR', BASE_DIR)
os.makedirs(DATA_DIR, exist_ok=True)

# Database configuration
DB_PATH = os.environ.get('DATABASE_PATH', os.path.join(DATA_DIR, 'attendance.db'))

# Upload configuration
UPLOAD_FOLDER = os.environ.get('UPLOAD_FOLDER', os.path.join(DATA_DIR, 'static', 'uploads'))
...
def init_database():
    ...
    try:
        ...
        cursor.execute("SELECT password FROM admin WHERE username = ?", (admin_username,))
        row = cursor.fetchone()
        if not row:
            hashed_pw = generate_password_hash(admin_password)
            cursor.execute("INSERT INTO admin (username, password) VALUES (?, ?)", (admin_username, hashed_pw))
        elif not row[0].startswith('pbkdf2:') and not row[0].startswith('scrypt:') and not row[0].startswith('argon2:'):
            hashed_pw = generate_password_hash(row[0])
            cursor.execute("UPDATE admin SET password = ? WHERE username = ?", (hashed_pw, admin_username))
...
def login_post():
    ...
    if connection:
        ...
        if row:
            stored_pw = row[0]
            is_valid = check_password_hash(stored_pw, password) if (stored_pw.startswith('pbkdf2:') or stored_pw.startswith('scrypt:') or stored_pw.startswith('argon2:')) else (stored_pw == password)
            if is_valid:
